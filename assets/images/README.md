...
- `projects/ai-startup-hq/` — low-res page gallery `ai-startup-hq-01.webp` through `05.webp`; high-res Dropbox-upload sources in `dropbox/` (`01`–`08`) plus intentional unsuitable shots (`unsuitable-01`…`04`). HTML must not reference `dropbox/`; HQ stays under `assets/` so unlinked same-origin orphans remain discoverable. See `IMAGE-PROMPTS.md`.
